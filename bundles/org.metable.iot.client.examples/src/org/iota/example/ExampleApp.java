...
public class ExampleApp {
    ...
    public static void createMaxDust() {
        ...
        String[] addresses = iota.getAddresses(seed)
                // We start from index 1 so we can send remaining balance to the address with
                // index 0
                .withRange(1, 101).finish();

        ClientMessageBuilder message_builder = iota.message().withSeed(seed);
        for (String address : addresses) {
            // Make sure to re-set the builder as the instance is a clone of the old one due
            // to JNI limits
        ...
        // At this point we have 100 Mi on 100 addresses and we will just send it to the
        // final address
        ...
        // only check last message, if this gets confirmed all other messages should
        // also be confirmed
